...
- Name: context-menu.spec.ts >> builds a fence from the build menu
- Location: tests/context-menu.spec.ts:167:0
  ...
  - region "Zoo state" [ref=e4]:
    - generic [ref=e5]:
      - generic [ref=e6]:
        ...
        - paragraph [ref=e9]: "Server: dev-world-1781907806054315019 v9"
  ...
  - complementary "Build menu" [ref=e116]:
    - generic [ref=e117]:
      - generic [ref=e118]:
        ...
        - paragraph [ref=e120]: placement rule is not met
    ...
    - list [ref=e123]:
      ...
      - list [ref=e136]:
        ...
        - button "Place Restroom" [ref=e158] [cursor=pointer]:
          - generic [ref=e160]:
            - generic [ref=e161]:
    ...
    - generic "Map builder" [ref=e316]:
      ...
      - button "Build Fence" [pressed] [ref=e332] [cursor=pointer]:
        - generic [ref=e333]: Build Fence
        - generic [ref=e334]: F
      - button "Confirm Fence" [active] [ref=e335] [cursor=pointer]:
...
```ts
  97  |     await page.mouse.move(end.x, end.y, { steps: 8 });
  98  |     await page.mouse.up();
  99  |     if (await confirmPath.isEnabled()) break;
  100 |   }
  101 | 
  102 |   await expect(confirmPath).toBeEnabled();
  103 |   await confirmPath.click();
  104 | 
  105 |   await expect(page.locator("#inspector-title")).toHaveText("Staff Path 1");
  106 |   await expect(page.locator("#inspector-details")).toContainText("Player drawn");
  107 |   await expect(page.locator("#inspector-details")).toContainText("Staff Path");
  108 |   await expect(page.locator("#build-menu-status")).toHaveText("Staff Path 1 built.");
  109 | });
  110 | 
  111 | test("draws a new path starting from clear ground", async ({ page }) => {
  112 |   await startZoo(page);
  113 |   await page.getByRole("button", { name: "Place building" }).click();
  114 |   await page.getByRole("button", { name: "Service Path" }).click();
  115 |   await page.getByRole("button", { name: "Draw Path" }).click();
  116 | 
